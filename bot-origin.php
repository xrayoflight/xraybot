...
if ($data == "editInviteAmount" && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    delMessage();
    sendMessage("Пожалуйста, введите сумму комиссии в Руб.ах", $cancelKey);
    setUser($data);
}
...
if ($data == "inviteFriends") {
    ...
    if ($inviteText != null) {
        ...
        sendMessage("Пригласите своих друзей в робота по вышеуказанной ссылке, и за каждую покупку вы получите $inviteAmount", json_encode(['inline_keyboard' => [[['text' => $buttonValues['back_to_main'], 'callback_data' => "mainMenu"]]]]), null, null, $msgId);
    } else
...
if ($data == "increaseMyWallet") {
    delMessage();
    sendMessage("🙂 Пожалуйста, введите сумму пополнения в Руб.ах (более 5000 Руб.)", $cancelKey);
    setUser($data);
}
...
if ($userInfo['step'] == "increaseMyWallet" && $text != $buttonValues['cancel']) {
    if (!is_numeric($text)) {
        ...
        sendMessage("Пожалуйста, введите сумму более 5000");
        exit();
    }
    ...
    sendMessage("Информация о пополнении:\nСумма " . number_format($text) . " Руб.\n\n Пожалуйста,выберите метод оплаты\n Внимание! \n В данный момент оплата возможна только в криптовалюте через платежный шлюз NowPayment либо прямым переводом на кошелек TRON ", $keys);
    setUser();
}
                ...
                if(!isset($list[0]->clientStats)){
                    foreach($list as $keys=>$packageInfo){
                    	if(strpos($packageInfo->settings, $text)!=false){
                            ...
                            $expiryTime = $packageInfo->expiryTime != 0?jdate("Y-m-d H:i:s",substr($packageInfo->expiryTime,0,-3)):"Неограничено";
                    ...
                    if(!isset($clientsSettings[$settingKey]['email'])){
                        ...
                        if(count($clientState) > 1){
                            ...
                            $expiryTime = $expTime != 0?jdate("Y-m-d H:i:s",substr($expTime,0,-3)):"Неограничено";
                            $expiryDay = $expTime != 0?
                                floor(
                            ...
                            $expiryTime = $expiry != 0?jdate("Y-m-d H:i:s",substr($expiry,0,-3)):"Неограничено";
                            $expiryDay = $expiry != 0?
                                floor(
