...
if(isset($update->message)){
    ...
    $username = $update->message->from->username?? " не отправлено ";
...
if(isset($update->callback_query)){
    ...
    $username = $update->callback_query->from->username?? " не отправлено ";
...
function getMainKeys(){
    ...
    array_push($mainKeys,$temp);
    if($from_id == $admin || $userInfo['isAdmin'] == true) array_push($mainKeys,[['text'=>"Управление ботом ⚙️",'callback_data'=>"managePanel"]]);
...
function getAgentKeys(){
    ...
    $agencyDate = date("Y-m-d H:i:s",$userInfo['agent_date']);
    $joinedDate = date("Y-m-d H:i:s",$userInfo['date']);
...
function getAdminKeys(){
    ...
    return json_encode(['inline_keyboard'=>[
        ...
        [
            ...
            ['text'=>'Отклоненные запросы','callback_data'=>"rejectedAgentList"]
        ],
        [['text'=>$buttonValues['back_to_main'],'callback_data'=>"mainMenu"]],
    ]]);
...
function getRejectedAgentList(){
    ...
    if($list->num_rows>0){
        $keys = array();
        $keys[] = [['text'=>"Освободить",'callback_data'=>"wizwizch"],['text'=>"Имя пользователя",'callback_data'=>'wizwizch'],['text'=>"Цифровой ID",'callback_data'=>"wizwizch"]];
...
function getAgentsList($offset = 0){
    ...
    $keys[] = [
        ...
        ['text' => "Процент скидки", 'callback_data' => "wizwizch"],
        ['text' => "Дата представительства", 'callback_data' => "wizwizch"],
        ['text' => "Имя представителя", 'callback_data' => "wizwizch"],
        ['text' => "Цифровой идентификатор", 'callback_data' => "wizwizch"]
    ];
        ...
        while($row = $agentList->fetch_assoc()){
            ...
            $fullName = $userDetail->first_name . " " . $userDetail->last_name;
            $joinedDate = date("Y-m-d H:i",$row['agent_date']);
    ...
    if ($offset == 0 && $limit <= $agentList->num_rows) {
        $keys[] = [
            ['text' => " »» Следующая страница »»", 'callback_data' => "nextAgentList" . ($offset + $limit)]
        ];
    } elseif ($limit <= $agentList->num_rows) {
        $keys[] = [
            ['text' => " »» Следующая страница »»", 'callback_data' => "nextAgentList" . ($offset + $limit)],
            ['text' => " «« Предыдущая страница ««", 'callback_data' => "nextAgentList" . ($offset - $limit)]
        ];
    } elseif ($offset != 0) {
        $keys[] = [
            ['text' => " «« Предыдущая страница ««", 'callback_data' => "nextAgentList" . ($offset - $limit)]
...
function getAgentDiscounts($agentId){
    ...
    ['text' => "Общее", 'callback_data' => "wizwizch"]];    
    ...
    if ($botState['agencyPlanDiscount'] == "on") {
        $keys[] = [['text' => "Добавить скидку на план", 'callback_data' => "addDiscountPlanAgent" . $agentId]];
    } else {
        $keys[] = [['text' => "Добавить скидку на сервер", 'callback_data' => "addDiscountServerAgent" . $agentId]];
    }
...
function getServerConfigKeys($serverId,$offset = 0){
    ...
    $portType = ($serverConfig['port_type'] == "auto") ? "Автоматически" : "Случайный";
    $serverType = " ";
    ...
    return json_encode(['inline_keyboard'=>[
        ...
        [
            ...
            ['text'=>"❕Имя сервера",'callback_data'=>"wizwizch"]
        ],
        [
            ['text'=>$flagwizwiz,'callback_data'=>"editServerFlag$id"],
            ['text'=>"🚩 Флаг сервера",'callback_data'=>"wizwizch"]
        ],
        [
            ['text'=>$remarkwizwiz,'callback_data'=>"editServerRemark$id"],
            ['text'=>"📣 Комментарий к серверу",'callback_data'=>"wizwizch"]
        ],
        [
            ['text'=>$serverType??" ",'callback_data'=>"changeServerType$id"],
            ['text'=>"Тип сервера",'callback_data'=>"wizwizch"]
        ],
        [
            ['text'=>$portType,'callback_data'=>"changePortType$id"],
            ['text'=>"Тип порта",'callback_data'=>"wizwizch"]
        ],
        [
            ['text'=>$ucount,'callback_data'=>"editServerMax$id"],
            ['text'=>"Вместимость сервера",'callback_data'=>"wizwizch"]
        ],
        [
        ...
        [
            ...
            ['text'=>"Тип заголовка",'callback_data'=>"wizwizch"],
        ],
        [
            ['text'=>$requestHeader,'callback_data'=>"editsServerrequest_header$id"],
            ['text'=>"Заголовок запроса",'callback_data'=>"wizwizch"],
        ],
        [
            ['text'=>$responseHeader,'callback_data'=>"editsServerresponse_header$id"],
            ['text'=>"Заголовок ответа",'callback_data'=>"wizwizch"],
        ],
        [
            ['text'=>$security,'callback_data'=>"editsServersecurity$id"],
            ['text'=>"Безопасность",'callback_data'=>"wizwizch"],
        ],
        (($serverConfig['type'] == "sanaei" || $serverConfig['type'] == "alireza")?
        [
            ['text'=>$reality,'callback_data'=>"changeRealityState$id"],
            ['text'=>"Реальность",'callback_data'=>"wizwizch"],
        ]:[]),
        [
            ['text'=>"♻️ Изменить IP сервера",'callback_data'=>"changesServerIp$id"],
        ],
        [
            ['text'=>"♻️ Изменить настройки безопасности",'callback_data'=>"editsServertlsSettings$id"],
        ],
        [
            ['text'=>"🔅Изменить данные для входа",'callback_data'=>"changesServerLoginInfo$id"],
        ],
        [
            ['text'=>"✂️ Удалить сервер",'callback_data'=>"wizwizdeleteserver$id"],
        ],
...
function getServerListKeys($offset = 0){
    ...
    $keys = array();
    $keys[] = [['text'=>"Статус",'callback_data'=>"wizwizch"],['text'=>"Настройки",'callback_data'=>"wizwizch"],['text'=>"Тип",'callback_data'=>"wizwizch"],['text'=>"Сервер",'callback_data'=>"wizwizch"]];
    
    if($cats->num_rows == 0){
        $keys[] = [['text'=>"Серверы не найдены",'callback_data'=>"wizwizch"]];
    }else {
        while($cty = $cats->fetch_assoc()){
            ...
            $stmt->close(); 
            $portType = $serverTypeInfo['port_type']=="auto"?"Авто":"Случайный";
            $serverType = " ";
    ...
    if($offset == 0 && $cats->num_rows >= $limit){
        $keys[] = [['text'=>" »» Следующая страница »»",'callback_data'=>"nextServerPage" . ($offset + $limit)]];
    }
    elseif($cats->num_rows >= $limit){
        $keys[] = [
            ['text'=>" »» Следующая страница »»",'callback_data'=>"nextServerPage" . ($offset + $limit)],
            ['text'=>" «« Предыдущая страница ««",'callback_data'=>"nextServerPage" . ($offset - $limit)]
        ];
    }
    elseif($offset != 0){
        $keys[] = [['text'=>" «« Предыдущая страница ««",'callback_data'=>"nextServerPage" . ($offset - $limit)]];
    }
    
    $keys[] = [['text'=>'🪙 Добавить новый сервер','callback_data'=>"addNewServer"]];
    $keys[] = [['text' => $buttonValues['back_button'], 'callback_data' => "managePanel"]];
    return json_encode(['inline_keyboard'=>$keys]);
}
...
function getCategoriesKeys($offset = 0){
    ...
    $keys = array();
    $keys[] = [['text'=>"Удалить",'callback_data'=>"wizwizch"],['text'=>"Имя категории",'callback_data'=>"wizwizch"]];
    
    if($cats->num_rows == 0){
        $keys[] = [['text'=>"Категории не найдены",'callback_data'=>"wizwizch"]];
    } else {
        while($cty = $cats->fetch_assoc()){
            $id = $cty['id'];
            $cname = $cty['title'];
    ...
    if($offset == 0 && $cats->num_rows >= $limit){
        $keys[] = [['text'=>" »» Следующая страница »»",'callback_data'=>"nextCategoryPage" . ($offset + $limit)]];
    }
    elseif($cats->num_rows >= $limit){
        $keys[] = [
            ['text'=>" »» Следующая страница »»",'callback_data'=>"nextCategoryPage" . ($offset + $limit)],
            ['text'=>" «« Предыдущая страница ««",'callback_data'=>"nextCategoryPage" . ($offset - $limit)]
        ];
    }
    elseif($offset != 0){
        $keys[] = [['text'=>" «« Предыдущая страница ««",'callback_data'=>"nextCategoryPage" . ($offset - $limit)]];
    }
    
    $keys[] = [['text'=>'➕ Добавить новую категорию','callback_data'=>"addNewCategory"]];
    $keys[] = [['text' => $buttonValues['back_button'], 'callback_data' => "managePanel"]];
    return json_encode(['inline_keyboard'=>$keys]);
}
...
function getGateWaysKeys(){
    ...
    return json_encode(['inline_keyboard'=>[
        ...
        [
            ...
            ['text'=>"Канал отчетности о доходах",'callback_data'=>'wizwizch']
        ],
        [
            ['text'=>$lockChannel,'callback_data'=>'editLockChannel'],
            ['text'=>"Заблокированный канал",'callback_data'=>'wizwizch']
        ],
...
function getBotSettingKeys(){
    ...
    $updateConnectionState = $botState['updateConnectionState']=="robot"?"Из робота":"Из сайта";
    $rewaredTime = ($botState['rewaredTime']??0) . " часов";
    switch($botState['remark']){
        case "digits":
            $remarkType = "Случайное число из 5 цифр";
            break;
        case "manual":
            $remarkType = "Пользовательский";
            break;
        default:
            $remarkType = "ID и случайное число";
            break;
    }
...
function getBotReportKeys(){
    ...
    $stmt->execute();
    $totalRewards = number_format($stmt->get_result()->fetch_assoc()['total']) . " Руб.";
    $stmt->close();
    
    
    $persian = explode("-",date("Y-n-1", time()));
    ...
    $stmt->execute();
    $monthReward = number_format($stmt->get_result()->fetch_assoc()['total']) . " Руб.";
    $stmt->close();
    ...
    $stmt->bind_param("i", $dayTime);
    $stmt->execute();
    $weekReward = number_format($stmt->get_result()->fetch_assoc()['total']) . " Руб.";
    $stmt->close();
    ...
    $stmt->bind_param("i", $dayTime);
    $stmt->execute();
    $dayReward = number_format($stmt->get_result()->fetch_assoc()['total']) . " Руб.";
    $stmt->close();
    
    return json_encode(['inline_keyboard'=>[
        [
            ['text'=>$allUsers,'callback_data'=>'wizwizch'],
            ['text'=>"Всего пользователей",'callback_data'=>'wizwizch']
        ],
        [
            ['text'=>$allOrders,'callback_data'=>'wizwizch'],
            ['text'=>"Всего заказов",'callback_data'=>'wizwizch']
        ],
        [
            ['text'=>$allServers,'callback_data'=>'wizwizch'],
            ['text'=>"Всего серверов",'callback_data'=>'wizwizch']
        ],
        [
            ['text'=>$allCategories,'callback_data'=>'wizwizch'],
            ['text'=>"Всего категорий",'callback_data'=>'wizwizch']
        ],
        [
            ['text'=>$allPlans,'callback_data'=>'wizwizch'],
            ['text'=>"Всего планов",'callback_data'=>'wizwizch']
        ],
        [
            ['text'=>$totalRewards,'callback_data'=>'wizwizch'],
            ['text'=>"Общий доход",'callback_data'=>'wizwizch']
        ],
        [
        ...
        [
            ['text'=>"Вернуться к управлению",'callback_data'=>'managePanel']
        ]
    ]]);
...
function getAdminsKeys(){
    ...
    if($usersList->num_rows > 0){
        ...
        $keys[] = [['text'=>"Список администраторов пуст ❕",'callback_data'=>"wizwizch"]];
    }
    $keys[] = [['text'=>"➕ Добавить администратора",'callback_data'=>"addNewAdmin"]];
    $keys[] = [['text'=>$buttonValues['back_button'],'callback_data'=>"managePanel"]];
    return json_encode(['inline_keyboard'=>$keys]);
}
function getUserInfoKeys($userId){
    ...
    if($userCount->num_rows > 0){
        ...
        $userWallet = number_format($userInfos['wallet']) . " Руб.";
        ...
        $totalBoughtPrice = number_format($info['total']) . " Руб.";
        ...
        return json_encode(['inline_keyboard'=>[
            ...
            [
                ...
                ['text'=>"Имя",'callback_data'=>"wizwizch"]
            ],
            [
                ['text'=>$boughtService??" ",'callback_data'=>"wizwizch"],
                ['text'=>"Сервисы",'callback_data'=>"wizwizch"]
            ],
            [
                ['text'=>$totalBoughtPrice??" ",'callback_data'=>"wizwizch"],
                ['text'=>"Сумма покупок",'callback_data'=>"wizwizch"]
            ],
            [
                ['text'=>$userWallet??" ",'callback_data'=>"wizwizch"],
                ['text'=>"Баланс кошелька",'callback_data'=>"wizwizch"]
            ],
            [
...
function getDiscountCodeKeys(){
    ...
    $keys = array();
    if($list->num_rows > 0){
        $keys[] = [['text'=>'❌','callback_data'=>"wizwizch"],['text'=>"Использование каждого пользователя",'callback_data'=>"wizwizch"],['text'=>"Дата окончания",'callback_data'=>"wizwizch"],['text'=>"Количество использований",'callback_data'=>"wizwizch"],['text'=>"Сумма скидки",'callback_data'=>"wizwizch"],['text'=>"Код скидки",'callback_data'=>"wizwizch"]];
        while($row = $list->fetch_assoc()){
            $date = $row['expire_date']!=0?date("Y/n/j H:i", $row['expire_date']):"Неограничено";
            $count = $row['expire_count']!=-1?$row['expire_count']:"Неограничено";
            $amount = $row['amount'];
            $amount = $row['type'] == 'percent'? $amount."%":$amount = number_format($amount) . " Руб.";
            $hashId = $row['hash_id'];
            $rowId = $row['id'];
            $canUse = $row['can_use'];
        ...
        $keys[] = [['text'=>"Скидочных кодов не найдено",'callback_data'=>"wizwizch"]];
    }
    
    $keys[] = [['text'=>"Добавить промокод",'callback_data'=>"addDiscountCode"]];
    $keys[] = [['text'=>$buttonValues['back_button'],'callback_data'=>"managePanel"]];
    return json_encode(['inline_keyboard'=>$keys]);
}
function getMainMenuButtonsKeys(){
    ...
    if($buttons->num_rows > 0){
        ...
        $keys[] = [['text'=>"Кнопка не найдена ❕",'callback_data'=>"wizwizch"]];
    }
    $keys[] = [['text'=>"Добавить новую кнопку ➕",'callback_data'=>"addNewMainButton"]];
    $keys[] = [['text'=>$buttonValues['back_button'],'callback_data'=>"managePanel"]];
    return json_encode(['inline_keyboard'=>$keys]);
}
function getPlanDetailsKeys($planId){
    ...
    if($pdResult->num_rows == 0) return null;
        ...
        $srvid= $pd['server_id'];
        $keyboard = [
            ($rahgozar==true?[['text'=>"* Тип плана: Рахгозар *",'callback_data'=>'wizwizch']]:[]),
            ($rahgozar==true?[
                ['text'=>$customPath,'callback_data'=>'changeCustomPath' . $id],
                ['text'=>"Пользовательский путь",'callback_data'=>'wizwizch'],
                ]:[]),
            ($rahgozar==true?[
                ['text'=>$customPort,'callback_data'=>'changeCustomPort' . $id],
                ['text'=>"Пользовательский порт",'callback_data'=>'wizwizch'],
                ]:[]),
            ($rahgozar==true?[
                ['text'=>$customSni,'callback_data'=>'changeCustomSni' . $id],
                ['text'=>"Пользовательский SNI",'callback_data'=>'wizwizch'],
                ]:[]),
            [['text'=>$name,'callback_data'=>"wizwizplanname$id"],['text'=>"🔮 Имя плана",'callback_data'=>"wizwizch"]],
            ($reality == "true"?[['text'=>$dest,'callback_data'=>"editDestName$id"],['text'=>"Dest",'callback_data'=>"wizwizch"]]:[]),
            ($reality == "true"?[['text'=>$serverName,'callback_data'=>"editServerNames$id"],['text'=>"Server Names",'callback_data'=>"wizwizch"]]:[]),
            ...
            [['text'=>$wizwizplanaccnumber,'callback_data'=>"wizwizch"],['text'=>"🎗 Количество проданных аккаунтов",'callback_data'=>"wizwizch"]],
            ($pd['inbound_id'] != 0?[['text'=>"$acount",'callback_data'=>"wizwizplanslimit$id"],['text'=>"🚪 Изменить емкость конфигурации",'callback_data'=>"wizwizch"]]:[]),
            ($pd['inbound_id'] != 0?[['text'=>$pd['inbound_id'],'callback_data'=>"wizwizplansinobundid$id"],['text'=>"🚪 Строка конфигурации",'callback_data'=>"wizwizch"]]:[]),
            [['text'=>"✏️ Редактировать описание",'callback_data'=>"wizwizplaneditdes$id"]],
            [['text'=>number_format($price) . " Руб.",'callback_data'=>"wizwizplanrial$id"],['text'=>"💰 Стоимость плана",'callback_data'=>"wizwizch"]],
            [['text'=>"♻️ Получить список аккаунтов",'callback_data'=>"wizwizplanacclist$id"]],
            [['text'=>"✂️ Удалить",'callback_data'=>"wizwizplandelete$id"]],
                ...
                if($security == "xtls"){
                    $keyboard = [
                        ...
                        [
                            ...
                            ['text' => "Покупка агента", 'callback_data' => "wizwizch"],
                        ],
                        [
