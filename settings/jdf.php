    ...
    if ($m === '') {
      ...
      if ($s === '') {
        ...
        if ($jm === '') {
          ...
          $date = explode('_', date('Y_j', '', '', $timezone, 'en'));
          if ($jd === '') {
            list($gy, $gm, $gd) = jalali_to_gregorian($date[0], $jm, $date[1]);
            return mktime($h, $m, $s, $gm);
          } else {
            if ($jy === '') {
              list($gy, $gm, $gd) = jalali_to_gregorian($date[0], $jm, $jd);
...
function jgetdate($timestamp = '', $none = '', $timezone = 'Asia/Tehran', $tn = 'en') {
  ...
  $date = explode('_', date('F_G_i_j_l_n_s_w_Y_z', $ts, '', $timezone, $tn));
  return array(
    'seconds' => tr_num((int) tr_num($date[6]), $tn),
    'minutes' => tr_num((int) tr_num($date[2]), $tn),
    'hours' => $date[1],
    'mday' => $date[3],
    'wday' => $date[7],
    'mon' => $date[5],
