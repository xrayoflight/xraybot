...
if($giftList->num_rows>0){
    ...
    if ($offset == 0 && $server_offset == 0) sendMessage("Начата операция подарка объема и времени для сервера $serverName", null, null, $admin);
        ...
        if(isset($row->clientStats)){
            foreach($clients as $key => $client) {
                ...
                if(!is_null($response)){
                    ...
                    if($orderExist->num_rows > 0){
                        ...
                        $gift = "";
                        if ($volume != 0) $gift = "$volume мегабайт объема ";
                        if($day != 0){
                            $expire_date = $day * 86400;
                            $gift .= " и $day дней времени";
                        ...
                        sendMessage("К вашей подписке $clientRemark было добавлено $gift", null, null, $user_id);
                    }
                }
            ...
            if(!is_null($response)){
                ...
                if($orderExist->num_rows > 0){
                    ...
                    sendMessage("В вашу подписку $remark было добавлено $gift", null, null, $user_id);
                }
            }
            $rowCount++;
        }
    ...
    if($found == false){
        ...
        sendMessage("Операция подарка объема и времени для сервера $serverName завершена", null, null, $admin);
    }
}
