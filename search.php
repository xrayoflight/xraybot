...
if(!file_exists("baseInfo.php") || !file_exists("config.php")){
    form("Не удалось найти необходимые файлы");
    exit();
}
...
if(isset($_REQUEST['id'])){
    $config_link = $_REQUEST['id'];
        ...
        if(preg_match('/^vmess:\/\/(.*)/',$config_link,$match)){
            ...
            && !(preg_match('/^[a-zA-Z0-9]{5,15}/',$config_link))){
                form("Введенный текст недействителен");
                exit();
        }
                ...
                if(!isset($list[0]->clientStats)){
                    foreach($list as $keys=>$packageInfo){
                        if(strpos($packageInfo->settings, $config_link)!=false){
                            ...
                            $state = $packageInfo->enable == true ? "Активен 🟢" : "Неактивен 🔴";
                            $totalUsed = sumerize2($packageInfo->up + $packageInfo->down);
                            $total = $packageInfo->total!=0?sumerize2($packageInfo->total):"Неограничено";
                            $expiryTime = $packageInfo->expiryTime != 0?date("Y-m-d H:i", strtotime(jdate("Y-m-d H:i",substr($packageInfo->expiryTime,0,-3)))):"Неограничено";
                            $leftMb = $packageInfo->total!=0?sumerize2($packageInfo->total - $packageInfo->up - $packageInfo->down):"Неограничено";
                    ...
                    if(!is_array($clientsSettings)){
                        form("К сожалению, произошла ошибка. Пожалуйста, повторите попытку.");
                        exit();
                    }
                    ...
                    if(!isset($clientsSettings[$settingKey]['email'])){
                        ...
                        if($clientState[$emailKey]->total != 0 || $clientState[$emailKey]->up != 0  ||  $clientState[$emailKey]->down != 0 || $clientState[$emailKey]->expiryTime != 0){
                            ...
                            $total = $total!=0?sumerize2($total):"Неограничено";
                            $expTime = $clientState[$emailKey]->expiryTime == 0 && $list[$keys]->expiryTime?$list[$keys]->expiryTime:$clientState[$emailKey]->expiryTime;
                            $expiryTime = $expTime != 0?date("Y-m-d H:i", strtotime(jdate("Y-m-d H:i",substr($expTime,0,-3)))):"Неограничено";
                            $expiryDay = $expTime != 0?
                                floor(
                            ...
                            $state = $clientState[$emailKey]->enable == true ? "Активен 🟢" : "Неактивен 🔴";
                            $remark = $email;
                        }
                        elseif($list[$keys]->total != 0 || $list[$keys]->up != 0  ||  $list[$keys]->down != 0 || $list[$keys]->expiryTime != 0){
                            ...
                            $leftMb = $list[$keys]->total!=0?($list[$keys]->total - $list[$keys]->up - $list[$keys]->down):"Неограничено";
                            if(is_numeric($leftMb)){
                                if($leftMb<0){
                                    $leftMb = 0;
                                }else{
                            ...
                            $total = $list[$keys]->total!=0?sumerize2($list[$keys]->total):"Неограничено";
                            $expiryTime = $list[$keys]->expiryTime != 0?date("Y-m-d H:i", strtotime(jdate("Y-m-d H:i",substr($list[$keys]->expiryTime,0,-3)))):"Неограничено";
                            ...
                            $state = $list[$keys]->enable == true ? "Активен 🟢" : "Неактивен 🔴";
    ...
    if(!$found){
        form("Введенные данные неверны", $cancelKey);
    }else{
        showForm("configInfo");
    }
}
...
function showForm($type){
    ...
    <head>
        ...
        <title><?php if($type=="unknown") echo "Поиск информации о конфигурации"; ?>
        elseif ($type=="id") echo "Результаты информации о конфигурации";
            ?></title>
        <meta charset="utf-8">
    ...
    <?php if ($type=="configInfo"){
        $download = $download != 0 && $total != "Неограничено"? round(100 * $download / $total,2):0;
        $upload = $upload != 0 && $total != "Неограничено"? round(100 * $upload / $total,2):0;
        $leftMb = $leftMb != "Неограничено" && $total != "Неограничено"?round(100 * $leftMb / $total,2):"100";
        ?>
        <div class="container" style="">
            <form id="contact" class="contactw">
                <br>
                <p style="font-size:22px; font-weight: bold; color:#1d3557; font-family:Arial, sans-serif !important;"> (Информация о конфигурации <?php echo $remark;?> ) </p>
                <p style="font-size:18px; font-weight: bold; color:#1d3557; margin-top:15px;"> Состояние: <?php echo $state;?> </p>
                ...
                <div class="mainform" >
                    ...
                    <p style="font-size:16px;">Объем загрузки</p>
                    ...
                    <div style="margin-right:50px;">
                        ...
                        <p style="font-size:16px; font-family:Arial !important;">Размер загрузки</p>
                ...
                <div class="mainform" style="margin-top:50px;">
                    ...
                    <div style="margin-left: 6px">
                        <svg style="margin-left: 6px" xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width="20" height="20"><path d="M23.9,11.437A12,12,0,0,0,0,13a11.878,11.878,0,0,0,3.759,8.712A4.84,4.84,0,0,0,7.113,23H16.88a4.994,4.994,0,0,0,3.509-1.429A11.944,11.944,0,0,0,23.9,11.437Zm-4.909,8.7A3,3,0,0,1,16.88,21H7.113a2.862,2.862,0,0,1-1.981-.741A9.9,9.9,0,0,1,2,13,10.014,10.014,0,0,1,5.338,5.543,9.881,9.881,0,0,1,11.986,3a10.553,10.553,0,0,1,1.174.066,9.994,9.994,0,0,1,5.831,17.076ZM7.807,17.285a1,1,0,0,1-1.4,1.43A8,8,0,0,1,12,5a8.072,8.072,0,0,1,1.143.081,1,1,0,0,1,.847,1.133.989.989,0,0,1-1.133.848,6,6,0,0,0-5.05,10.223Zm12.112-5.428A8.072,8.072,0,0,1,20,13a7.931,7.931,0,0,1-2.408,5.716,1,1,0,0,1-1.4-1.432,5.98,5.98,0,0,0,1.744-5.141,1,1,0,0,1,1.981-.286Zm-5.993.631a2.033,2.033,0,1,1-1.414-1.414l3.781-3.781a1,1,0,1,1,1.414,1.414Z"/></svg>
                        <p style="font-size:16px; font-family:Arial, sans-serif !important;">Оставшийся объем</p>
                    ...
                    <div style="margin-right:50px;">
                        ...
                        <p style="font-size:16px;">Общий объем</p>
    ...
    elseif($type=="unknown"){ ?>
        ...
        <div class="container">
            ...
            <h3 style="margin:20px;">Пожалуйста, введите запрошенную информацию</h3>
                <fieldset>
                <input placeholder="Введите ссылку на соединение или идентификатор конфигурации (uuid)" type="text" id="id" name="id" autocomplete="off" required>
                </fieldset>
                <fieldset>
