...
if (mysqli_num_rows($payInfo) == 0) {
    showForm("Платеж с указанным кодом не найден", "Ошибка!");
} else {
    ...
    $days = $payParam['day'];
    if ($payType == "BUY_SUB") $payDescription = "Покупка аккаунта";
    elseif ($payType == "RENEW_ACCOUNT") $payDescription = "Продление аккаунта";
    elseif ($payType == "RENEW_SCONFIG") $payDescription = "Продление аккаунта";
    elseif ($payType == "INCREASE_WALLET") $payDescription = "Пополнение кошелька";
    elseif (preg_match('/^INCREASE_DAY_(\d+)_(\d+)/', $payType)) $payDescription = "Увеличение срока аккаунта";
    elseif (preg_match('/^INCREASE_VOLUME_(\d+)_(\d+)/', $payType)) $payDescription = "Увеличение объема аккаунта";
        ...
        if ($res->payment_status == 'finished' or $res->payment_status == 'confirmed' or $res->payment_status == 'sending') {
            ...
            if ($res->payment_status == 'partially_paid') {
                ...
                showForm("#$hash_id - Вы внесли меньшую сумму, пожалуйста, обратитесь в поддержку", $payDescription);
            } else {
                ...
                showForm("Платеж не выполнен", $payDescription);
            }
        }
    ...
    showForm("Платеж не выполнен", "Ошибка!");
}
}
...
if($payType == "BUY_SUB"){
    ...
    if($acount == 0 and $inbound_id != 0){
        showForm('پرداخت شما انجام شد ولی ظرفیت این کانکشن پر شده است، مبلغ ' . number_format($amount) . " Руб. به کیف پول شما اضافه شد",$payDescription, false);
        
        $stmt = $connection->prepare("UPDATE `users` SET `wallet` = `wallet` + ? WHERE `userid` = ?");
        $stmt->bind_param("ii", $amount, $user_id);
        $stmt->execute();
        $stmt->close();
        sendMessage("✅ مبلغ " . number_format($amount). " Руб. به حساب شما اضافه شد",null,null,$user_id);
        sendMessage("✅ مبلغ " . number_format($amount) . " Руб. به کیف پول کاربر $user_id توسط درگاه اضافه شد میخواست کانفیگ بخره، ظرفیت پر بود",null,null,$admin);                
    ...
    if($inbound_id == 0) {
        ...
        if($server_info['ucount'] <= 0) {
            showForm('پرداخت شما انجام شد ولی ظرفیت این سرور پر شده است، مبلغ ' . number_format($amount) . " Руб. به کیف پول شما اضافه شد",$payDescription, false);
    ...
    for($i =1; $i<= $accountCount; $i++){
        ...
        if(is_null($response)){
            ...
            $stmt->close();
            sendMessage("✅ مبلغ " . number_format($amount). " Руб. به حساب شما اضافه شد",null,null,$user_id);
            sendMessage("✅ مبلغ " . number_format($amount) . " Руб. به کیف پول کاربر $user_id توسط درگاه اضافه شد میخواست کانفیگ بخره، اتصال به سرور برقرار نبود",null,null,$admin);                
    	...
    	if($response == "inbound not Found"){
            ...
            $stmt->close();
            sendMessage("✅ مبلغ " . number_format($amount). " Руб. به حساب شما اضافه شد",null,null,$user_id);
            sendMessage("✅ مبلغ " . number_format($amount) . " Руб. به کیف پول کاربر $user_id توسط درگاه اضافه شد میخواست کانفیگ بخره، ولی انباند پیدا نشد",null,null,$admin);                
    
    		exit;
    	}
    	if(!$response->success){
            showForm('پرداخت شما با موفقیت انجام شد ولی خطا داد لطفا سریع به مدیر بگو ... مبلغ '. number_format($amount) . " Руб. به کیف پولت اضافه شد",$payDescription);
            ...
            $stmt->close();
            sendMessage("✅ مبلغ " . number_format($amount). " Руб. به حساب شما اضافه شد",null,null,$user_id);
            sendMessage("✅ مبلغ " . number_format($amount) . " Руб. به کیف پول کاربر $user_id توسط درگاه اضافه شد میخواست کانفیگ بخره، ولی خطا داد",null,null,$admin);                
            exit;
        }
...
🧝‍♂️ID пользователя: $user_id
🛡Имя пользователя: $first_name
🔖 نام کاربری: $username
💰مبلغ پرداختی: $amount Руб.
🔮 نام سرویس: $remark
🔋حجم سرویس: $volumeГигабайт
⏰ مدت سرویس: $days روز
⁮⁮ 
...
elseif($payType == "INCREASE_WALLET"){
    ...
    $stmt->execute(); 
    $stmt->close(); 
    showForm("پرداخت شما با موفقیت انجام شد، مبلغ ". number_format($amount) . " Руб. به کیف پول شما اضافه شد",$payDescription, true);
    sendMessage("✅ مبلغ " . number_format($amount). " Руб. به حساب شما اضافه شد",null,null,$user_id);
    sendMessage("✅ مبلغ " . number_format($amount) . " Руб. به کیف پول کاربر $user_id توسط درگاه اضافه شد",null,null,$admin);                
}
elseif($payType == "RENEW_ACCOUNT"){
	...
	if(is_null($response)){
		showForm('پرداخت شما با موفقیت انجام شد ولی مشکل فنی در اتصال به سرور. لطفا به مدیریت اطلاع بدید، مبلغ ' . number_format($amount) . " Руб. به کیف پول شما اضافه شد",$payDescription);
        ...
        $stmt->close();
        sendMessage("✅ مبلغ " . number_format($amount). " Руб. به حساب شما اضافه شد",null,null,$user_id);
        sendMessage("✅ مبلغ " . number_format($amount) . " Руб. به کیف پول کاربر $user_id اضافه شد، میخواست کانفیگش رو تمدید کنه، ولی اتصال به سرور برقرار نبود",null,null,$admin);
		exit;
	}
...
exit;
        ...
        showForm("پرداخت شما با موفقیت انجام شد ولی به دلیل مشکل فنی امکان افزایش حجم نیست. لطفا به مدیریت اطلاع بدید یا 5دقیقه دیگر دوباره تست کنید مبلغ " . number_format($amount) . " Руб. به کیف پول شما اضافه شد", $payDescription, true);
        $stmt = $connection->prepare("UPDATE `users` SET `wallet` = `wallet` + ? WHERE `userid` = ?");
        $stmt->bind_param("ii", $amount, $user_id);
        $stmt->execute();
        $stmt->close();
        sendMessage("✅ مبلغ " . number_format($amount). " Руб. به حساب شما اضافه شد",null,null,$user_id);
        sendMessage("✅ مبلغ " . number_format($amount) . " Руб. به کیف پول کاربر $user_id اضافه شد، میخواست زمان سرویسشو افزایش بده",null,null,$admin);
        exit;
    }
}
elseif(preg_match('/^INCREASE_VOLUME_(\d+)_(\d+)/',$payType, $match)){
    ...
    if($response->success){
        ...
        $stmt->execute();
        $stmt->close();
        showForm("پرداخت شما با موفقیت انجام شد. $volumeГигабайт به حجم سرویس شما اضافه شد",$payDescription, true);
        $keys = json_encode(['inline_keyboard'=>[
        [
            ['text'=>"خرید از درگاه $gateType 💞",'callback_data'=>'wizwizch'],
            ],
...
exit;
        ...
        showForm("پرداخت شما با موفقیت انجام شد ولی مشکل فنی در ارتباط با سرور. لطفا سلامت سرور را بررسی کنید مبلغ " . number_format($amount) . " Руб. به کیف پول شما اضافه شد",$payDescription, true);
        ...
        $stmt->close();
        sendMessage("✅ مبلغ " . number_format($amount). " Руб. به حساب شما اضافه شد",null,null,$user_id);
        sendMessage("✅ مبلغ " . number_format($amount) . " Руб. به کیف پول کاربر $user_id اضافه شد، میخواست حجم کانفیگشو افزایش بده",null,null,$admin);                
